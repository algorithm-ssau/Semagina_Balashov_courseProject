        ...
        for corr_elem, test_elem in zip(corr_sent, test_sents[i]):
            ...
            if (corr_pos == test_pos) or (corr_pos == "NOUN" and test_pos == "PROPN"):
                ...
                if tag_match and measure_lemmas and test_lemma is not None:
                    ...
                    lemma_match = False
                with open("engine/test/table/correct.txt", "a", encoding="UTF-8") as f:
                    f.write(word + ":" + corr_pos + ":" + test_pos + "\n")

            else:
                tag_match, lemma_match = False, False
                with open("engine/test/table/incorrect.txt", "a", encoding="UTF-8") as f:
                    f.write(word + ":" + corr_pos + ":" + test_pos + "\n")
...
def measure(corr_file, test_file, measure_lemmas, dump_file):
    ...
    print("{} частей речи из {} верны, точность {:.2f}%".format(correct_pos, total_tags, pos_accuracy))
    full_accuracy = None
